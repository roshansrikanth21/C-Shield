...
function createRouter2(init) {
  ...
  async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
    ...
    if (isRedirectResult2(result)) {
      ...
      if (opts && opts.replace != null) {
        ...
        let location2 = normalizeRedirectLocation2(result.response.headers.get("Location"), new URL(request.url), basename);
        replace2 = location2 === state.location.pathname + state.location.search;
      }
      await startRedirectNavigation(request, result, true, {
        submission,
  ...
  async function startRedirectNavigation(request, redirect3, isNavigation, _temp2) {
    ...
    location = normalizeRedirectLocation2(location, new URL(request.url), basename);
...
function normalizeRedirectLocation2(location, currentUrl, basename) {
  if (ABSOLUTE_URL_REGEX2.test(location)) {
    let normalizedLocation = location;
    let url = normalizedLocation.startsWith("//") ? new URL(currentUrl.protocol + normalizedLocation) : new URL(normalizedLocation);
